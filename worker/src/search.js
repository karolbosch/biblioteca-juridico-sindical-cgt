...
const fields="d.id,d.doc_id,d.title,d.document_type,d.source_type,d.sector,d.court_level,d.resolution_number,d.date,d.year,d.company,d.matter,d.submatter,d.outcome,d.procedural_status,d.privacy_status,d.source_url,d.document_status,d.chain_id,d.final_authority,d.current_rule_summary,d.summary,d.criteria,d.pdf_public_path,d.public_path,d.page";
...
export function ruleBasedAnswer(question,sources,otherSectorLabel){
  ...
  const legalText=legal?.current_rule_summary||legal?.summary||legal?.title,agreementText=agreement?.current_rule_summary||agreement?.summary;
  let baseLegal;
  if(legal&&agreement)baseLegal=`Estatuto de los Trabajadores («${legal.title}»): ${legalText}\n\nConvenio colectivo («${agreement.title}»): ${agreementText}\n\nNinguna de las dos fuentes puede aplicarse si empeora lo que reconozca la otra: compare ambas cifras y aplique la que sea más favorable para la persona trabajadora.`;
  else if(legal)baseLegal=`Estatuto de los Trabajadores («${legal.title}»): ${legalText}. No consta que el convenio colectivo regule este mismo supuesto en la biblioteca.`;
  else if(agreement)baseLegal=`Convenio colectivo («${agreement.title}»): ${agreementText}. No consta en la biblioteca que el Estatuto de los Trabajadores mejore este punto.`;
  else baseLegal="El dato no consta en la biblioteca.";
  const jurisprudenceText=caselaw?`${caselaw.title}: ${caselaw.criteria||caselaw.current_rule_summary||caselaw.summary||"criterio no detallado en la biblioteca"}.`:"No consta jurisprudencia específica sobre esta cuestión en la biblioteca.";
