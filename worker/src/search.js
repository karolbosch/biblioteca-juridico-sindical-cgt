...
export async function retrieve(env,question,filters={},limit=12){
  ...
  if(filters.sectorHint){
    try{const{results:sectorDocs}=await env.DB.prepare(`SELECT ${fields} FROM documents d WHERE d.privacy_status IN ('PUBLICABLE','ANONIMIZACION_VERIFICADA') AND (d.sector=? OR d.sector='laboral-general') LIMIT 300`).bind(filters.sectorHint).all();const known=new Set(results.map(r=>r.id));for(const doc of sectorDocs)if(!known.has(doc.id)){results.push(doc);known.add(doc.id)}}catch{}
  }
  const questionCategory=categorizeTopic({title:question});
  const semanticScores=filters.semantic===false?new Map():await semanticRank(env,question,results.map(r=>r.id)).catch(()=>new Map());
  return results.filter(doc=>(!filters.sector||doc.sector===filters.sector)&&(!filters.source_type||doc.source_type===filters.source_type)&&(!filters.otherSectorLabel||!(doc.document_type==="convenio"||doc.source_type==="CONVENIO"))).map(doc=>{let bonus=0;if(filters.sectorHint){if(doc.sector===filters.sectorHint){bonus+=8;if(filters.subsectorHint){if(doc.submatter===filters.subsectorHint)bonus+=20;else if(doc.submatter)bonus-=8}}else if(doc.sector==="laboral-general")bonus+=6;else if(doc.sector)bonus-=25}else if(doc.sector==="laboral-general")bonus+=4;const category=categorizeTopic(doc);if(isCaselaw(doc)&&category===questionCategory&&questionCategory!=="otras-materias")bonus+=6;const semanticScore=semanticScores.get(doc.id)||0;if(semanticScore>0.55)bonus+=(semanticScore-0.5)*30;return{...doc,category,semanticScore,termMatch:termMatchCount(doc,question),rank:legalRank(doc,question)+bonus}}).sort((a,b)=>b.rank-a.rank).slice(0,Math.min(limit,30))
}
