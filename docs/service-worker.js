const CACHE="consultas-juridico-sindical-v8";
const CORE=["./","./index.html","./styles.css","./themes.css","./pwa.css","./features.css","./config.js","./app.js","./pwa.js","./manifest.webmanifest","./icons/icon.svg","./data/documents.json","./data/chains.json","./admin/","./admin/index.html","./admin/admin.js","./sector/contact-center/"];
...
if(/\.(js|css)$/.test(url.pathname)){event.respondWith(fetch(request).then(response=>{if(response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(request,copy))}return response}).catch(()=>caches.match(request)));return}
